package cn.verlu.lulu.feature.lifestream.data.repository

import android.content.Context
import android.graphics.Bitmap
import android.graphics.BitmapFactory
import android.net.Uri
import cn.verlu.lulu.feature.lifestream.core.log.MemoryLog
import cn.verlu.lulu.data.local.dao.MemoryEntryDao as LuluMemoryEntryDao
import cn.verlu.lulu.data.local.entity.MemoryEntryEntity as LuluMemoryEntryEntity
...
class FileLifeStreamRepository(
    ...
    private val luluMemoryEntryDao: LuluMemoryEntryDao,
    ...
    override fun observeEntries(): Flow<List<LifeEntry>> =
        combine(
            memoryEntryDao.observeAll(),
            luluMemoryEntryDao.observeEntries(),
        ) { timelineEntries, luluMemories ->
            mergeById(
                timelineEntries.map { it.toDomain() },
                luluMemories.map { it.toLifeEntry() },
            )
        }
    ...
    override suspend fun getAll(): List<LifeEntry> = mutex.withLock {
        ...
        val cloudSyncEnabled = settingsStore.isCloudSyncEnabled()
    ...
    private fun LuluMemoryEntryEntity.toLifeEntry(): LifeEntry = LifeEntry(
        id = id,
        content = buildString {
            if (title.isNotBlank() && title != content.take(32)) {
                append(title)
                append("\n")
            }
            append(content)
        ...
        syncState = when (syncStatus) {
            MemorySyncStatus.SYNCED.name -> SyncState.SYNCED
            MemorySyncStatus.FAILED.name -> SyncState.ERROR
            else -> SyncState.LOCAL_ONLY
        },
        retryCount = 0,
        type = LifeEntryType.TEXT,
        mediaList = emptyList(),
    )
